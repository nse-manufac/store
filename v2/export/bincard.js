...
import { KINDS, isKnownKind } from '../core/ledger.js';
 ...
 * v1 มีแค่ IN กับ OUT ส่วน v2 มีแปดชนิด ฟอร์มมีแค่สองฝั่ง จึงต้องเลือกฝั่งจากผลที่มีต่อยอด
...
export function toCardLines(rows, unit = '') {
  let no = 0;
  return rows.map(r => {
    const known = isKnownKind(r.kind);
    const label = (KINDS[r.kind] || {}).label || r.kind;
    const extra = [];
    // ชนิดที่โปรแกรมรุ่นนี้ไม่รู้จัก ถูกคิดเป็นศูนย์และลงฝั่งรับเข้า — ต้องบอกคนอ่านการ์ดว่าบรรทัดนี้เชื่อไม่ได้
    if (!known) extra.push(`⚠ ชนิด "${r.kind}" ที่โปรแกรมรุ่นนี้ไม่รู้จัก — บรรทัดนี้ไม่ถูกนับเข้ายอด`);
    else if (r.kind !== 'receive' && r.kind !== 'issue') extra.push(label);
...
/** คำเตือนที่พิมพ์ลงการ์ด — คนที่รับการ์ดต่อไม่ได้เห็นหน้าจอของเรา ต้องอ่านเจอในกระดาษเอง */
export const UNKNOWN_KIND_NOTE = 'การ์ดนี้ออกจากโปรแกรมรุ่นเก่า — มีรายการที่โปรแกรมไม่รู้จักและไม่ได้นับเข้ายอด '
  + 'ยอดคงเหลือในการ์ดนี้ไม่ถูกต้อง ให้โหลดโปรแกรมใหม่แล้วออกการ์ดใหม่';

/**
 * ออกการ์ดหนึ่งใบ — ทางเดียวที่ app.js ใช้ ห้ามเรียก writeCard ตรง ๆ (v2-export หมวด D ตรวจอยู่)
 *
 * writeCard ต้องเหมือน v1 ทุกบรรทัด (v2-export หมวด C) จึงแก้ข้างในไม่ได้
 * คำเตือนเรื่องชนิดที่ไม่รู้จักจึงเขียนทับหลังจากนั้น ที่แถวว่างเหนือตาราง (B3) กับสองแถวใต้แถวรวม
 * ทั้งสองจุดอยู่ในพื้นที่พิมพ์ที่ applyTpl ตั้งไว้ (ถึงแถวรวม + 3)
 * คืนจำนวนบรรทัดที่ไม่รู้จัก ให้หน้าจอบอกคนกดด้วย
 */
export function writeBinCard(ws, info, lines) {
  ...
  const text = `⚠ ${UNKNOWN_KIND_NOTE} (${n} รายการ)`;
  for (const addr of ['B3', 'B' + (BINCARD_TPL.firstRow + R + 2)]) {
    const c = ws.getCell(addr);
    c.value = text;
    c.font = { name: 'Tahoma', size: 11, bold: true, color: { argb: 'FFC00000' } };
  }
  return n;
}
