...
createApp({
  setup() {
     ...
     *
     * ⚠️ ใช้ดัชนีไม่ใช่ matOf เพราะช่องนี้ถูกเรียก **ทุกแถวทุกครั้งที่เรนเดอร์**
     * ทะเบียนของจริงหมื่นสองพันแถว · ตารางเดียวร้อยแถวก็ไล่เทียบล้านครั้งต่อการเรนเดอร์หนึ่งรอบ
     * (ผู้ตรวจ #100 ทักไว้สองรอบ) · ดัชนีเก็บตัวแรกที่เจอ ให้ผลเท่ากับ find() เดิมเมื่อรหัสซ้ำ
     */
    const matIndex = computed(() => {
      const m = new Map();
      for (const x of materials.value) {
        const k = normCode(x.material_code);
    ...
    async function onInFile(e) {
      ...
      e.target.value = '';
      if (!file) return;
      // ⚠️ กด [ไปรับของ] จากแท็บซื้อทดแทนมา แล้วเลือกไฟล์ทับ = บรรทัดที่ปุ่มใส่ไว้หายทั้งบรรทัด
      // แล้ว linkBuy หารหัสไม่เจอ เรื่องซื้อทดแทนจะไม่ถูกปิด โดยไม่มีอะไรฟ้อง (ผู้ตรวจ #104)
      // ไม่แก้ด้วยการงอกบรรทัดคืน เพราะบรรทัดนั้นไม่มี PO แล้วด่านข้างล่างจะบล็อกทั้งใบ
      // จึงถามก่อนทับ — ให้คนเลือกเองว่าจะรับเรื่องซื้อทดแทนให้จบก่อน หรือเลิกรอ
      if (buyWaits.value.length && !confirm(
        [`มีเรื่องซื้อทดแทนรอผูกกับใบรับเข้าอยู่ ${buyWaits.value.length} เรื่อง`,
         'กางไฟล์ตอนนี้จะทับบรรทัดนั้นทิ้ง และเรื่องจะไม่ถูกปิดให้อัตโนมัติ',
         'ถ้าจะรับของเรื่องนั้น ให้กดบันทึกให้จบก่อนแล้วค่อยเลือกไฟล์',
         '', 'กางไฟล์ทับไหม'].join('\n'))) return;
    ...
    async function saveIn() {
      ...
      if (noPo.length) {
        ...
        // ทางคีย์เองเปิดอยู่แล้ว ปุ่ม "คีย์เอง" จะหายไป — ห้ามบอกให้ไปกดปุ่มที่ไม่มีบนจอ (ผู้ตรวจ #104)
        flash(`มี ${noPo.length} บรรทัดที่ยังไม่รู้ว่าเป็นของ PO ไหน`
          + (inManual.value ? ' — ใส่เลข PO ที่ช่องบนหัวจอ'
    ...
    // ⚠️ ไม่มีช่อง "กลุ่ม" (H / U) แล้ว (เจ้าของสั่งเอาออก 22 ก.ย. 2026)
    // ไม่มีโค้ดไหนอ่านค่านั้นเลย ไม่ได้ลงไปในรายการที่บันทึก และนิติบุคคลดูจากเลขที่ PO รายบรรทัดอยู่แล้ว
    // เดิมมีปุ่มดึงจาก Kit List เป็นที่เดียวที่เติมค่าให้ พอปุ่มถูกลบก็เหลือเป็นช่องที่กรอกแล้วค่าหายเฉย ๆ
    const wkH = reactive({ date: todayLocal(), person: '', entity: '' });
    const wkLines = ref([]);
    ...
    // ── นำเข้า PO / Kit List ───────────────────────────────────────
    // ไฟล์จาก Delta สามแบบ อ่านด้วยตัวอ่านคนละตัว
    // แต่หน้านี้รับเข้าท่อแค่สองแบบ — PO รายวัน กับ Kit List (22-H)
    // กลุ่มจ่ายรวมยังต้องเดาชนิดให้ออก แต่เด้งไปแท็บรับเข้ารวมรายรอบ (เจ้าของ 22 ก.ย. 2026)
    const pos = ref([]);
    const kits = ref([]);
    const shorts = ref([]);
     ...
     * ⚠️ ยังต้องเดาครบสามแบบแม้จะรับเข้าแค่สองแบบ — ถ้าเลิกเดาชนิดกลุ่มจ่ายรวม
     * ไฟล์แบบนั้นจะตกไปเข้าตัวอ่าน 22-H แล้วอ่านได้หน้าตาปกติโดยเอาเลขผู้ขายมาเป็นรหัสวัตถุดิบ
     */
     ...
     *
     * ⚠️ ยกเว้นการเดินกลับไปแท็บ fbuy — ปุ่ม [ไปรับของ] อยู่แท็บนั้นแท็บเดียว
     *    การจะกดเรื่องที่สองจึงบังคับให้ต้องผ่านทางนี้เสมอ ถ้าล้างคิวตรงนี้
     *    คิวจะมีได้ไม่เกินหนึ่งเรื่องตลอดกาล เรื่องแรกหลุดคิวเงียบ ๆ ทั้งที่บรรทัดยังค้างอยู่ในใบ
     *    แล้วพนักงานจะไปกด [ไปรับของ] ซ้ำแล้วคีย์รับเข้าอีกรอบ = ของกองเดียวเข้าคลังสองครั้ง
     *    (ผู้ตรวจรอบ 5 ข้อ 1) · ทางออกอื่นทุกทางยังเลิกรอให้เหมือนเดิม
     *    และยังมีปุ่ม "ไม่รับของแล้ว" ให้กดเลิกรอเองอยู่ */
    watch(tab, (now, before) => {
      if (before === 'in' && now !== 'in' && now !== 'fbuy') buyWaits.value = [];
    });
